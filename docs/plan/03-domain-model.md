...
Synchronized entities use stable IDs. Names, folder paths, hostnames, and display labels are not identity.
...
Use UUIDs or an equivalent opaque stable identifier for synchronized entities.
...
Folders support nesting, creation, deletion, renaming, movement, and synchronization.
...
Synchronized identity references should use cryptographic identity, not machine paths.
...
The model should allow machine-local overrides without altering synchronized records.
